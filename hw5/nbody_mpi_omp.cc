...
static std::mt19937* gen_ptr; // Mersenne twister engine
static std::uniform_real_distribution<>* ran_ptr; // Uniform distribution

static int thread; // Unique thread number
#pragma omp threadprivate(thread, gen_ptr, ran_ptr)
...
void setup_parallelism() {
    ...
    #pragma omp parallel 
    {
        ...
        gen_ptr = new std::mt19937;
        (*gen_ptr).seed(now_int ^ (thread * n_ranks + rank)); // Seed the random number generator
        ran_ptr = new std::uniform_real_distribution<>(0., 1.);
    }
...
std::tuple<Vec, Vec> initial_conditions() {
    ...
    for (int i = ND_beg; i < ND_end; ++i) {
        x[i] = (*ran_ptr)(*gen_ptr) * dx + x_min; // Random initial positions
        v[i] = (*ran_ptr)(*gen_ptr) * dv + v_min; // Random initial velocities
    }
...
std::tuple<Vec, Vec> timestep(const Vec& x0, const Vec& v0, const Vec& m) {
    ...
    if (n_ranks > 1) { // More than one process
        // Gather the initial positions and velocities
        MPI_Allgatherv(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, x1.data(), countsD.data(), displsD.data(), MPI_DOUBLE, MPI_COMM_WORLD);
        MPI_Allgatherv(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, v1.data(), countsD.data(), displsD.data(), MPI_DOUBLE, MPI_COMM_WORLD);
    }
